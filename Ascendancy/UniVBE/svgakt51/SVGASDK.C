...
*
* $Id: svgasdk.c 1.1 1995/02/06 16:26:07 kjb release $
*
****************************************************************************/
...
#define MAXMODES    60				/* Maximum modes available in list  */
...
PUBLIC	int     curBank;
PUBLIC	void 	(_cdecl *SV_putPixel)(int x,int y,ulong color);
PUBLIC	void 	(_cdecl *SV_line)(int x1,int y1,int x2,int y2,ulong color);
PUBLIC	void 	(_cdecl *SV_clear)(ulong color);
PUBLIC	void 	(_cdecl *SV_setActivePage)(int page);

PRIVATE bool    old50Lines;         /* Was old mode 80x50?              */
PRIVATE	int		oldGS;				/* Old value of GS selector			*/
PRIVATE	int		newGS;				/* Current GS selector				*/
...
void _cdecl VBE_setBankA(void);		/* VBE setBank routine single bank	*/
void _cdecl VBE_setBankAB(void);	/* VBE setBank routine dual banks	*/

#ifndef	REALMODE
void _cdecl VBE20_setBankA(void);	/* VBE 2.0 setBank single bank		*/
void _cdecl VBE20_setBankAB(void);	/* VBE 2.0 setBank dual banks		*/
...
int _cdecl _setGS(uint sel);
void _cdecl _setActivePage(int page);
void _cdecl VGA_setPalette(int start,int num,VBE_palette *pal,bool waitVRT);
void _cdecl VBE20_setPalette(int start,int num,VBE_palette *pal,bool waitVRT);
void _cdecl _clear16(ulong color);
void _cdecl _clear256(ulong color);
void _cdecl _clear32k(ulong color);
void _cdecl _clear16m(ulong color);
void _cdecl _clear4G(ulong color);
void _cdecl _putPixel16(int x,int y,ulong color);
void _cdecl _putPixel256(int x,int y,ulong color);
void _cdecl _putPixel32k(int x,int y,ulong color);
void _cdecl _putPixel16m(int x,int y,ulong color);
void _cdecl _putPixel4G(int x,int y,ulong color);
void _cdecl _line16(int x1,int y1,int x2,int y2,ulong color);
void _cdecl _line256(int x1,int y1,int x2,int y2,ulong color);
void _cdecl _line32k(int x1,int y1,int x2,int y2,ulong color);
void _cdecl _line16m(int x1,int y1,int x2,int y2,ulong color);
void _cdecl _line4G(int x1,int y1,int x2,int y2,ulong color);

/* Declare all the linear framebuffer routines */

#ifndef	REALMODE
#ifdef	PM286
void _cdecl linsdk_enable32(void);
#endif
void _cdecl _setActivePageLin(int page);
void _cdecl _clear256Lin(ulong color);
void _cdecl _clear32kLin(ulong color);
void _cdecl _clear16mLin(ulong color);
void _cdecl _clear4GLin(ulong color);
void _cdecl _putPixel256Lin(int x,int y,ulong color);
void _cdecl _putPixel32kLin(int x,int y,ulong color);
void _cdecl _putPixel16mLin(int x,int y,ulong color);
void _cdecl _putPixel4GLin(int x,int y,ulong color);
void _cdecl _line256Lin(int x1,int y1,int x2,int y2,ulong color);
void _cdecl _line32kLin(int x1,int y1,int x2,int y2,ulong color);
void _cdecl _line16mLin(int x1,int y1,int x2,int y2,ulong color);
void _cdecl _line4GLin(int x1,int y1,int x2,int y2,ulong color);
#endif
...
****************************************************************************/
{
	...
	if (modeInfo.MemoryModel == vbeMemRGB) {
	...
	oldGS = _setGS(newGS = PM_getVGAGraphSelector());
	...
	if (mode & vbeLinearBuffer) {
		ushort linSel = VBE_getLinearSelector(&modeInfo);
		if (linSel == 0) {
			SV_restoreMode();
			printf("Unable to obtain linear framebuffer selector!\n");
			exit(1);
			}
		_setGS(newGS = linSel);
...
PUBLIC void SV_resetGS(void)
/****************************************************************************
*
* Function:     SV_resetGS
*
* Description:  Resets the value of the GS selector to our graphics display
*				area. This is needed under Windows 3.1 full screen
*				sessions in 16 bit protected mode because GS is used as a
*				general scratch selector by Windows. For 32 bit apps, GS
*				is a valid selector so it wont be trashed (at least this
*				is what I believe).
*
****************************************************************************/
{
...
****************************************************************************/
{
	...
	_setGS(oldGS);					/* Restore value of GS selector	*/
#ifndef	REALMODE

    ...
    def get_arr_memb_info(self):
        ...
        if self.vu:
            ...
            if not (vu and vu.get_current_item(ida_hexrays.USE_KEYBOARD) and vu.item.is_citem()):
                ...
                return None
            e1 = vu.item.e
            if (e1._op != ida_hexrays.cot_memref) and (e1._op != ida_hexrays.cot_memptr):
                ida_kernwin.warning("e1 is not cot_memref nor cot_memptr")
                return None
            e0 = vu.cfunc.body.find_parent_of(e1)
            if e0._op == ida_hexrays.cot_idx:
                e2 = e0.cexpr.y
                if e2._op == ida_hexrays.cot_add:
                    e2 = e2.cexpr.y
                if e2._op != ida_hexrays.cot_num:
                    ida_kernwin.warning("e0 is cot_idx and e2 is not cot_num")
                    return None
                ami.idx = e2.get_const_value()
            e3 = e1.cexpr.x
            if e1._op == ida_hexrays.cot_memref:
                if (e3._op != ida_hexrays.cot_obj) and (e3._op != ida_hexrays.cot_memref) and (e3._op != ida_hexrays.cot_var):
